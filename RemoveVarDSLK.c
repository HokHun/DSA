#include <stdio.h>
#include <stdlib.h>
// xóa tất cả các phần tử x khỏi DSLK
// tạo node
struct Node {
    int data;
    struct Node* next;
...
int main() {
    ...
    printf("Cac node trong DSLK hien tai: ");
    showLinkedListElement(singlyLinkedList);
    int x;
    printf("Nhap vao cac node can xoa: ");
    scanf("%d", &x);
    int removedNode = removeAllX(&singlyLinkedList, x);
    if(removedNode > 0) {
        printf("DSLK sau khi xoa tat ca cac node %d: \n", x);
        showLinkedListElement(singlyLinkedList);
        printf("So luong node bi xoa khoi DSLK: %d\n", removedNode);
    } else {
        printf("Khong ton tai node can xoa trong DSLK!\n");
    }
    return 0;
}